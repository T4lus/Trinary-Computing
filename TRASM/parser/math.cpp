...
std::vector<Tryte> Parser::ADD(std::vector<std::string> _opMap, maps_t maps) {
	...
	if (args_type[1] == T_REGISTER & T_ADDRESS) 	memTab.push_back(op_tab["ADD"].value + 1);
...
std::vector<Tryte> Parser::SUB(std::vector<std::string> _opMap, maps_t maps) {
	...
	if (args_type[1] == T_REGISTER & T_ADDRESS) 	memTab.push_back(op_tab["SUB"].value + 1);
...
std::vector<Tryte> Parser::MUL(std::vector<std::string> _opMap, maps_t maps) {
	...
	if (args_type[1] == T_REGISTER & T_ADDRESS) 	memTab.push_back(op_tab["MUL"].value + 1);
...
std::vector<Tryte> Parser::DIV(std::vector<std::string> _opMap, maps_t maps) {
	...
	if (args_type[1] == T_REGISTER & T_ADDRESS) 	memTab.push_back(op_tab["DIV"].value + 1);
